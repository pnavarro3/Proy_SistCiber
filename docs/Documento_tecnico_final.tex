...
\node[lab] at (4.2,-2.35) {Estado visual};
\end{tikzpicture}
...
FTJ & Robot & Al finalizar el movimiento fÃ­sico propio. \\
...
\texttt{simulink/} & Modelo principal \texttt{ProyectoSC.slx} y funciones MATLAB/Simulink para emisión/recepción de mensajes e interpretación de tablero. \\
\texttt{docs/} & Documento técnico final de entrega: fuente LaTeX (\texttt{.tex}) y versión compilada (\texttt{.pdf}). \\
...
\texttt{src/Calibracion.py} & Python & Ajuste y calibración visual del sistema en entorno de desarrollo. \\
\texttt{simulink/SimulinkEmisor.m} & MATLAB & Construcción de tramas \texttt{INI}/\texttt{MOV} para transmisión TCP/IP. \\
\texttt{simulink/SimulinkReceptor.m} & MATLAB & Decodificación de mensajes \texttt{DEC}/\texttt{ACT}/\texttt{FTU}/\texttt{FTJ}/\texttt{FIN} y señales de control. \\
\texttt{simulink/ProyectoSC.slx} & Simulink & Modelo principal de supervisión y control en el PC central. \\
\texttt{docs/}\allowbreak\texttt{Documento\_tecnico\_final.tex} & LaTeX & Fuente del documento técnico final. \\
\texttt{docs/}\allowbreak\texttt{Documento\_tecnico\_final.pdf} & PDF & Versión compilada para entrega y revisión. \\
\texttt{config/posbackup.json} & JSON & Persistencia de poses de referencia del robot. \\
\bottomrule
\end{tabular}
