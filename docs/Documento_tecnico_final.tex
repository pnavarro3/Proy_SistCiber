...
La soluci\'on no se ha limitado a una descripci\'on abstracta del sistema, sino que se apoya en componentes concretos ya desarrollados: ComRobot.py como n\'ucleo de control de partida en el robot, Ned2\_ULL.py como conjunto de utilidades de configuraci\'on y persistencia de poses, y los bloques SimulinkEmisor.m y SimulinkReceptor.m como interfaz entre el PC central y el protocolo TCP/IP.
\par
...
\par
En el estado presente del proyecto se dispone de funcionamiento operativo de comunicaci\'on y ejecuci\'on de turnos en modo manual y autom\'atico, con protocolo TCP/IP activo, detecci\'on visual integrada y secuencias de movimiento pick-and-place estables. No obstante, el cierre autom\'atico de partida y la emisi\'on sistem\'atica de mensaje FIN en el flujo principal del robot siguen en consolidaci\'on de integraci\'on. El cierre final del documento se completar\'a con las evidencias experimentales restantes y con los resultados de la versi\'on definitiva de c\'odigo.
...
Los ficheros desarrollados para el PC del robot y el PC central permiten distinguir claramente las capas de responsabilidad del sistema. ComRobot.py concentra el bucle de partida, control de turnos, sockets y secuencias de movimiento; Ned2\_ULL.py agrupa utilidades de configuraci\'on y persistencia de poses; y los dos bloques de Simulink materializan la comunicaci\'on bidireccional con el modelo del PC central.
...
FIN res\textbackslash n & VIC / DER / EMP & Soportado por receptor; emisi\'on desde robot pendiente de cierre. \\
\bottomrule
\end{tabular}
...
FIN res & Robot & Definido en protocolo y receptor, pendiente de activaci\'on completa en bucle principal. \\
\bottomrule
\end{tabular}
...
Robot $\rightarrow$ PC central & FIN res & Resultado final de partida (VIC, DER o EMP), previsto en interfaz. \\
\bottomrule
\end{tabular}
...
La toma de decisi\'on en modo autom\'atico implementada en la versi\'on actual es heur\'istica y ligera. En fase de colocaci\'on toma el centro (casilla 5) cuando est\'a disponible en la primera jugada; en caso contrario selecciona una casilla libre aleatoria. En fase de movimiento selecciona de forma aleatoria una ficha propia y una casilla libre. Esta estrategia permite validar integraci\'on extremo a extremo con bajo coste computacional.
\par
El enunciado fija adem\'as objetivos temporales concretos: 15 s m\'aximo para recogida, 15 s para transporte y 10 s para decisi\'on autom\'atica. En el c\'odigo actual no existe instrumentaci\'on de tiempos ni verificaci\'on autom\'atica de dichas cotas; el comportamiento temporal se gobierna por esperas fijas en secuencias de movimiento y por coste m\'inimo de c\'alculo en la decisi\'on heur\'istica.

La mensajer\'ia de audio aparece como capacidad probada en scripts de apoyo y comentarios de operaci\'on, pero no forma parte del flujo activo de ComRobot.py en esta revisi\'on.
...
\midrule
Proceso de recogida de pieza & $\leq 15$ s & Secuencias deterministas con esperas fijas; medici\'on autom\'atica pendiente. \\
Proceso de transporte de pieza & $\leq 15$ s & Trayectorias desacopladas Z/X-Y; validaci\'on temporal en campo pendiente. \\
Toma de decisi\'on en modo AUT & $\leq 10$ s & Heur\'istica aleatoria de bajo coste; medici\'on formal pendiente. \\
\bottomrule
\end{tabular}
\caption{Gr\'afica tabulada de objetivos temporales y soluci\'on t\'ecnica adoptada.}
\end{table}

\subsection{Observaciones de consistencia detectadas en el c\'odigo actual}
Del an\'alisis directo de los archivos Python/MATLAB se identifican los siguientes puntos de integraci\'on a cerrar en la versi\'on final:
\begin{itemize}
  \item En ComRobot.py existen funciones para verificar victoria y l\'imite de turnos, pero actualmente no gobiernan el cierre del bucle principal de partida.
  \item El mensaje FIN est\'a soportado por SimulinkReceptor.m, pero no se emite de forma sistem\'atica desde el flujo principal del robot.
  \item SimulinkReceptor.m implementa mapeo de s\'imbolos \texttt{X}/\texttt{O} a valores de tablero; conviene consolidar y documentar en un unico lugar la convenci\'on sem\'antica de \texttt{rival} y \texttt{propio} para evitar ambig\"uedades.
  \item No hay telemetr\'ia integrada para trazabilidad temporal autom\'atica de recogida, transporte y decisi\'on.
\end{itemize}
...
\subsection{Estado observado en c\'odigo actual (auditor\'ia de implementaci\'on)}
La tabla siguiente refleja el estado t\'ecnico observado directamente en el c\'odigo activo del repositorio en esta revisi\'on.
...
\midrule
\endhead
000C-003 & Implementado & Subsistema de visi\'on del robot & Captura de imagen comprimida, extracci\'on workspace y segmentaci\'on HSV. \\
000C-025 & Implementado & ComRobot.py + SimulinkReceptor.m & Comunicaci\'on remota TCP/IP y parseo de eventos discretos. \\
000C-026 & Implementado & ComRobot.py & Intercambio PC--robot con sockets, mensajes TX/RX y terminador \texttt{\textbackslash n}. \\
000C-030 & Implementado & Inicializaci\'on del control del robot & L\'imite de velocidad de brazo fijado a 50 en la configuraci\'on de arranque. \\
000C-034 & Implementado & ComRobot.py & Fin de turno rival por lectura de entrada digital DI1. \\
000C-038 & Implementado & ComRobot.py & Mensajes codificados como ASCII/UTF-8 con salto de l\'inea final. \\
000C-009 & Parcial & ComRobot.py & Modo manual activo con recepci\'on MOV, sin validaciones de robustez completas. \\
000C-010 & Parcial & ComRobot.py & Modo autom\'atico activo con heur\'istica aleatoria; no minimax en versi\'on actual. \\
000C-036 & Pendiente & ComRobot.py & La funci\'on de victoria existe, pero no cierra el bucle principal de partida. \\
000C-037 & Pendiente & ComRobot.py & MAX\_TURNOS definido, pendiente de aplicaci\'on efectiva en flujo principal. \\
\bottomrule
\end{longtable}
}
...
000C-009 & Modo manual & Test de sistema & Ejecutar bater\'ia de partidas completas para cerrar validaci\'on formal de robustez y errores de entrada. & En implementaci\'on \\
000C-010 & Modo autom\'atico & An\'alisis + Test & Registrar decisiones autom\'aticas y evaluar calidad de jugada de la heur\'istica actual en campo. & En implementaci\'on \\
...
000C-022 & Mensajes de audio & Test & Integrar audio en flujo principal y validar reproducci\'on en estados clave: inicio, movimiento y fin de partida. & Pendiente \\
000C-023 & Actualizaci\'on por visi\'on & Test & Contrastar tras cada turno el estado detectado con el tablero real y cerrar cobertura estad\'istica. & En implementaci\'on \\
...
000C-034 & Fin de turno rival por bot\'on f\'isico & Test & Accionar bot\'on f\'isico y comprobar transici\'on al siguiente estado en bater\'ia repetida. & En implementaci\'on \\
\bottomrule
\end{longtable}
}
